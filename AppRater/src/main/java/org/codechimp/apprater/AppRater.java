...
public class AppRater {
     ...
     * Call this method at the end of your OnCreate method to determine whether
     * to show the rate prompt using the specified or default day, launch count
     * values with additional day and launch parameter for remind me later
     * option and checking if the version is changed or not
     * 
     * @param context
    ...
    public static void app_launched(Context context, int daysUntilPrompt, int launchesUntilPrompt,
            int daysForRemind, int launchesForRemind) {
    ...
    public static void app_launched(Context context, int daysUntilPrompt, int launchesUntilPrompt) {
        ...
        if (isVersionNameCheckEnabled) {
            if (!ratingInfo.getApplicationVersionName().equals(
                    prefs.getString(PREF_APP_VERSION_NAME, "none"))) {
        ...
        checkAndShowRateAlertDialog(context);
        commitOrApply(editor);
    }

    public static void checkAndShowRateAlertDialog(Context context) {
        SharedPreferences prefs = context.getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = prefs.edit();
    ...
    public static void incrementEventCount(final Context context) {
        ...
        SharedPreferences.Editor editor = prefs.edit();
        long event_count = prefs.getLong(PREF_EVENT_COUNT, 0) + 1;
    ...
    private static void showRateAlertDialog(final Context context,
        ...
        if (Build.VERSION.SDK_INT >= 11 && themeSet) {
            builder = new AlertDialog.Builder(context, (isDark ? AlertDialog.THEME_HOLO_DARK
